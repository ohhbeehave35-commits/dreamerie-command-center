...
#
# model: eleven_multilingual_v2 is the highest-quality but SLOWEST model -- it
#   was adding real dead air before the first word. turbo_v2_5 is the low-latency
#   sibling at near-identical quality for English, and it's the one that honours
#   the `speed` setting. eleven_flash_v2_5 is faster still if we want to trade
#   a little polish for speed.
...
async def synthesize(text: str) -> bytes:
    ...
    async with httpx.AsyncClient(timeout=30) as http:
        r = await http.post(url, headers=headers, json=full)
        if r.status_code == 422:
            print(f"[tts] ElevenLabs rejected expressive settings, retrying plain: {r.text[:300]}", flush=True)
            r = await http.post(url, headers=headers, json=minimal)
